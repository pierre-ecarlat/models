...
class FasterRCNNMetaArch(model.DetectionModel):
  ...
  def propose_boxes_only(self, preprocessed_inputs):
    ...
    the call to the _predict_second_stage, and including the postprocess for
    the rpn (usually done during the second stage).
    ...
    Returns:
      prediction_dict: a dictionary holding "raw" prediction tensors, see the
        description in the predict method.
      proposal_boxes_normalized: A float32 tensor with shape [batch_size,
        num_proposals, box_code_size] containing proposal boxes in
        normalized coordinates.
      proposal_scores: A float tensor with shape [batch_size, num_proposals] 
        representing the proposal objectness scores for all the boxes.
    ...
    proposal_boxes_normalized, proposal_scores, num_proposals = \
        self._postprocess_rpn(rpn_box_encodings, 
                              rpn_objectness_predictions_with_background,
                              anchors, image_shape)
    
    return prediction_dict, proposal_boxes_normalized, \
           proposal_scores, num_proposals
  ...
  def classify_proposals_only(self, rpn_features_to_crop, image_shape, 
    ...
    excepting the postprocess for the rpn. It also has an explicit high level
    RFCN check, which probably can be removed by creating a dedicated 
    classify_proposals_only method in its meta_architecture.

    Args:
      rpn_features_to_crop: A 4-D float32 tensor with shape [batch, height, 
        width, depth] representing image features to crop using the 
        proposals boxes.
      ...
      num_proposals: An int32 tensor of shape [batch_size] representing the
        ...
        which are not since we always pad the number of proposals to be
        `self.max_num_proposals` for each image.
      isRFCN: A boolean set to True if the current architecture is RFCN

    Returns:
      prediction_dict: a dictionary holding "raw" prediction tensors, see the
        description in the _predict_second_stage method.
    """
    if not isRFCN:
      rpn_features_to_crop = (
          self._compute_second_stage_input_feature_maps(
