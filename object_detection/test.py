...
####################################################################
# Create the tensorflow sessions for all the models
####################################################################
sessions = {}
for m in ensemble:
    sessions[m] = tf.Session()
    path_to_ckpt = osp.join(MODELS['base_models_dir'], MODELS[m]['ckp_dir'])
    ...
    if not DEBUG_MODE:
        ...
        saver.restore(sessions[m], path_to_latest_ckpt)
